...
export default class {
  constructor(
    ...
    this.WucApi = WucApi;
    this.HARDWARE_RAID_RULE_DEFAULT_NAME = HARDWARE_RAID_RULE_DEFAULT_NAME;
    this.SERVERSTATS_PERIOD_ENUM = SERVERSTATS_PERIOD_ENUM;
  ...
  addTaskFast(productId, task, scopeId) {
    ...
    this.Polling.addTaskFast(
      this.getTaskPath(productId, task.id),
      task,
      scopeId,
    )
      .then((state) => {
  ...
  addTask(productId, task, scopeId) {
    ...
    this.Polling.addTask(
      this.getTaskPath(productId, task.id || task.taskId),
      task,
      scopeId,
    )
      .then((state) => {
  ...
  getPointInterval(period) {
    switch (period) {
      case 'HOURLY': {
        return this.SERVERSTATS_PERIOD_ENUM.HOURLY;
      }
      case 'DAILY': {
        return this.SERVERSTATS_PERIOD_ENUM.DAILY;
      }
      case 'WEEKLY': {
        return this.SERVERSTATS_PERIOD_ENUM.WEEKLY;
      }
      case 'MONTHLY': {
        return this.SERVERSTATS_PERIOD_ENUM.MONTHLY;
      }
      case 'YEARLY': {
        return this.SERVERSTATS_PERIOD_ENUM.YEARLY;
      }
      default:
        return null;
    }
  ...
  buildMRTGResponse(arrayIn, pointInterval) {
    const response = {
      state: 'OK',
      messages: [],
      upload: {
    ...
    response.download.values = this.fillGaps(arrayIn[0].data);
    response.upload.values = this.fillGaps(arrayIn[1].data);
    return response;
  }
  ...
  aggregateMRTG(productId, mac, type, period) {
    return this.getSelected(productId).then((server) =>
      this.$q
        ...
        .then((results) => {
          const pointInterval = this.getPointInterval(period);
          return this.buildMRTGResponse(results, pointInterval);
        }),
    );
  }
  ...
  updateServiceMonitoringNotificationEmail(productId, opts) {
    return this.put(
      productId,
    ...
    return this.post(
      ...
      '{templateName}/partitionScheme/{schemeName}/hardwareRaid',
      {
        ...
        data: {
          disks,
          mode: raid,
          name: this.HARDWARE_RAID_RULE_DEFAULT_NAME,
          step: 1,
        },
        proxypass: true,
        urlPath: this.path.installationMe,
  ...
  getPartitionSchemeHardwareRaid(productId, templateName, schemeName) {
    return this.get(
      ...
      const index = indexOf(response, this.HARDWARE_RAID_RULE_DEFAULT_NAME);
      if (index !== -1) {
        return this.get(
          productId,
          '{templateName}/partitionScheme/{schemeName}/hardwareRaid/{name}',
