...
export default class {
  ...
  constructor(
    ...
    this.Server = Server;
    this.User = User;
    this.NO_AUTORENEW_COUNTRIES = NO_AUTORENEW_COUNTRIES;
    this.WEATHERMAP_URL = WEATHERMAP_URL;
  }
  ...
  $onInit() {
    ...
    this.$scope.$on('server.monitoring.reload', this.loadMonitoring.bind(this));
    ...
    this.$scope.hasAutoRenew = () => {
      ...
      if (this.NO_AUTORENEW_COUNTRIES.indexOf(this.user.ovhSubsidiary) === -1) {
        return this.$q
          .all({
          ...
          .then((results) => {
            ...
            this.$scope.autoRenewGuide =
              this.constants.urls[this.user.ovhSubsidiary].guides.autoRenew ||
    ...
    this.$scope.isEligibleForUpgrade = this.isEligibleForUpgrade.bind(this);
  ...
  isHousing(dedicatedServer) {
    return dedicatedServer.commercialRange === 'housing';
  }

  isEligibleForUpgrade() {
    return includes(ELIGIBLE_FOR_UPGRADE.SUBSIDIARIES, this.user.ovhSubsidiary);
  }
  ...
  loadServer() {
    ...
    return this.$q
      ...
      .then((data) => {
        ...
        this.$scope.isHousing = this.isHousing(this.server).bind(this);
      ...
      .then(
        ([models, allServiceMonitoring]) => {
          ...
          this.$scope.servicesStateLinks = {
            weathermap: this.WEATHERMAP_URL,
